...
LessonsWidget::LessonsWidget(DB::DataBase &db, const DB::Link_TeachPlan &link)
	:m_DB(db), m_Link(link), m_ParentId(-1), m_Model(ORM::Data::create(DB::g_ModelPlan)), m_ButtonBox(Gtk::BUTTONBOX_SPREAD), m_ListBox(m_Model)
{
...
{
	Glib::RefPtr<ORM::Data> model = ORM::Data::create(DB::g_ModelLessonTeacher);
