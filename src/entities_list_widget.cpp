...
EntitiesListWidget::EntitiesListWidget(DB::DataBase &db, const DB::Entity &ent)
	:m_DB(db), m_Entity(ent), m_Model(Gtk::ListStore::create(DB::g_ModelEntity)), m_ButtonBox(Gtk::BUTTONBOX_SPREAD), m_ListBox(m_Model)
{
	m_ListBox.set_headers_visible(true);
	if(&ent == &DB::g_Auditoriums)
	{
		m_Model = Gtk::ListStore::create(DB::g_ModelAud);
